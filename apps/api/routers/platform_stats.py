...
# ── Simple in-process cache (60-second TTL) ───────────────────────────────
...
async def get_platform_stats(db: AsyncSession = Depends(get_db)) -> PlatformStats:
    ...
    total_q = await db.execute(
        select(func.count(TaskDB.id)).where(TaskDB.status == "completed")
    )
    tasks_completed_total: int = total_q.scalar_one() or 0
    ...
    tasks_completed_today: int = today_q.scalar_one() or 0
    ...
    tasks_completed_this_week: int = week_q.scalar_one() or 0

    # ── Active workers last 30 days ────────────────────────────────────────
    # A "worker" here means any user who submitted/approved a task assignment
    ...
    # ── Avg completion time overall (ms) ──────────────────────────────────
    avg_ms_q = await db.execute(
        select(func.avg(TaskDB.duration_ms)).where(
            TaskDB.status == "completed",
            TaskDB.duration_ms.isnot(None),
            TaskDB.duration_ms > 0,
        )
    )
    avg_raw = avg_ms_q.scalar_one()
    avg_completion_ms_overall: Optional[int] = int(avg_raw) if avg_raw else None
