...
@app.post("/v1/debug/fix-migrations", tags=["health"])
async def debug_fix_migrations():
    """Temporary endpoint: apply missing migrations 0061 + 0062. REMOVE AFTER FIX."""
    from sqlalchemy import text
    results: dict = {"steps": []}
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            if current == "0062":
                results["message"] = "Already at 0062, nothing to do"
                return results

            # ── Apply migration 0061: leagues ──────────────────────────────
            try:
                # Create enum type
                await session.execute(text(
                    "DO $$ BEGIN CREATE TYPE league_season_status_enum "
                    "AS ENUM ('active', 'processing', 'completed'); "
                    "EXCEPTION WHEN duplicate_object THEN NULL; END $$;"
                ))
                results["steps"].append({"step": "create_enum", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "create_enum", "ok": False, "error": str(e)})
            ...
            try:
                ...
                await session.execute(text("""
                    CREATE TABLE IF NOT EXISTS league_seasons (
                        ...
                        status league_season_status_enum NOT NULL DEFAULT 'active',
                        created_at TIMESTAMPTZ NOT NULL DEFAULT now()
                    )
                """))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_league_seasons_week_start ON league_seasons (week_start)"
                ))
                results["steps"].append({"step": "create_league_seasons", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "create_league_seasons", "ok": False, "error": str(e)})
            ...
            try:
                ...
                await session.execute(text("""
                    CREATE TABLE IF NOT EXISTS league_groups (
                        ...
                        created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                        CONSTRAINT uq_league_group UNIQUE (season_id, tier, group_number)
                    )
                """))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_league_groups_season_id ON league_groups (season_id)"
                ))
                results["steps"].append({"step": "create_league_groups", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "create_league_groups", "ok": False, "error": str(e)})
            ...
            try:
                ...
                await session.execute(text("""
                    CREATE TABLE IF NOT EXISTS league_group_members (
                        ...
                        result VARCHAR(16),
                        created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                        CONSTRAINT uq_league_group_member UNIQUE (group_id, user_id)
                    )
                """))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_league_group_members_group_id ON league_group_members (group_id)"
                ))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_league_group_members_user_id ON league_group_members (user_id)"
                ))
                results["steps"].append({"step": "create_league_group_members", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "create_league_group_members", "ok": False, "error": str(e)})
            ...
            try:
                ...
                results["steps"].append({"step": "add_league_tier_column", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "add_league_tier_column", "ok": False, "error": str(e)})

            # ── Apply migration 0062: quests ───────────────────────────────
            try:
                await session.execute(text("""
                ...
                """))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_active_quests_week_start ON active_quests (week_start)"
                ))
                results["steps"].append({"step": "create_active_quests", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "create_active_quests", "ok": False, "error": str(e)})

            try:
                await session.execute(text("""
                    CREATE TABLE IF NOT EXISTS quest_progress (
                        id UUID PRIMARY KEY,
                        quest_id UUID NOT NULL REFERENCES active_quests(id) ON DELETE CASCADE,
                        user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                        ...
                        extra_data JSON,
                        created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
                        CONSTRAINT uq_quest_progress UNIQUE (quest_id, user_id)
                    )
                """))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_quest_progress_quest_id ON quest_progress (quest_id)"
                ))
                await session.execute(text(
                    "CREATE INDEX IF NOT EXISTS ix_quest_progress_user_id ON quest_progress (user_id)"
                ))
                results["steps"].append({"step": "create_quest_progress", "ok": True})
            except Exception as e:
                results["steps"].append({"step": "create_quest_progress", "ok": False, "error": str(e)})

            # ── Update alembic version ─────────────────────────────────────
            try:
                await session.execute(text("UPDATE alembic_version SET version_num = '0062'"))
                await session.commit()
                results["steps"].append({"step": "update_alembic_version", "ok": True})
                results["new_version"] = "0062"
                results["message"] = "Migrations applied successfully"
            except Exception as e:
                results["steps"].append({"step": "update_alembic_version", "ok": False, "error": str(e)})
                results["message"] = "Failed to update alembic version"
